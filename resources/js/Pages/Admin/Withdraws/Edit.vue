<script setup>
	...
	const form = useForm({
		user_id: props.withdraw.user_id,
		uuid: props.withdraw.uuid,
		gateway: props.withdraw.gateway,
		remoteId: props.withdraw.remoteId,
		to: props.withdraw.to,
		gross_amount: props.withdraw.gross_amount,
		fees: props.withdraw.fees,
		amount: props.withdraw.amount,
		data: props.withdraw.data,
		status: props.withdraw.status,
	});
	const save = () =>
		form.put(window.route("withdraws.edit", props.withdraw.id));
</script>
<template>
	<AdminLayout>
		<Head :title="title ?? `Edit Withdraw`" />
		<main class="h-full">
			<div
