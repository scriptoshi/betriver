<script setup>
	...
	const deleteBetForm = useForm({});
	const betBeingDeleted = ref(null);

	const deleteBet = () => {
		deleteBetForm.delete(
			window.route("admin.bets.destroy", betBeingDeleted.value?.id),
			{
				preserveScroll: true,
				preserveState: true,
				onSuccess: () => (betBeingDeleted.value = null),
			},
		);
	};
	debouncedWatch(
	...
	const toggle = (bet) => {
		bet.busy = true;
		router.put(
			window.route("admin.bets.toggle", bet.id),
			{},
			{
				preserveScroll: true,
				preserveState: true,
...
<template>
	...
	<AdminLayout>
		...
		<ConfirmationModal
			:show="betBeingDeleted"
			@close="betBeingDeleted = null">
			<template #title>
				{{
					$t("Are you sure about deleting {bet} ?", {
						bet: betBeingDeleted.name,
					})
				}}
			</template>
			...
			<template #content>
				<p>
					{{
						$t(
							"This Action will remove the bet from the database and cannot be undone",
						)
					}}
				</p>
				<p>
					{{ $t("Its Recommended to Disable the bet Instead") }}
				</p>
			</template>
			...
			<template #footer>
				...
				<PrimaryButton
					...
					v-if="betBeingDeleted.active"
					@click="toggle(betBeingDeleted)">
					<Loading v-if="betBeingDeleted.busy" />
					{{ $t("Disable") }}
				</PrimaryButton>
				...
				<PrimaryButton
					...
					class="ml-2 uppercase text-xs font-semibold"
					@click="deleteBet"
					:class="{ 'opacity-25': deleteBetForm.processing }"
					:disabled="deleteBetForm.processing">
					{{ $t("Delete") }}
				</PrimaryButton>
			</template>
		</ConfirmationModal>
