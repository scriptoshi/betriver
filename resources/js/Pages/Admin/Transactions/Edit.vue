<script setup>
	...
	const form = useForm({
		...
		transactable: props.transaction.transactable,
		uuid: props.transaction.uuid,
		description: props.transaction.description,
		amount: props.transaction.amount,
		balance_before: props.transaction.balance_before,
		action: props.transaction.action,
		type: props.transaction.type,
	});
	const save = () =>
		form.put(window.route("transactions.edit", props.transaction.id));
</script>
<template>
	<AdminLayout>
		<Head :title="title ?? `Edit Transaction`" />
		<main class="h-full">
			<div
				...
				<div class="flex flex-col gap-4 h-full">
					<div
						...
						<div class="mb-4 lg:mb-0">
							<h3 class="h3">Edit Transaction</h3>
						</div>
						<div
							...
							<Link
								...
								{{ $t("Go Back to transactions list") }}
							</Link>
						</div>
					</div>
