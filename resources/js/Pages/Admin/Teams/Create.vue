<script setup>
	...
	const save = () => form.post(window.route("admin.teams.store"));
</script>
<template>
	<Head :title="title ?? `New Team`" />
	<AdminLayout>
		<main class="h-full">
			<div
					...
					<div
						...
						<div
							...
							<PrimaryButton
								link
								secondary
								:href="route('admin.teams.index', sport)">
								<VueIcon
									:icon="HiArrowLeft"
									class="w-4 h-4 -ml-2 mr-2 inline-block" />
								{{ $t("Go Back to teams list") }}
							</PrimaryButton>
						</div>
					</div>
							...
							<form class="space-y-8" @submit.prevent="save()">
								<div class="grid sm:grid-cols-3 gap-4">
									...
									<FormInput
										...
										type="text"
										placeholder="MANU"
										help="Abbreviated Name"
										:error="form.errors.code" />
								</div>

								<FormInput
									label="Description"
									placeholder="Top notch football team out of Manchester UK"
