<script setup>
	...
	const form = useForm({
		commission_id: props.payout.commission_id,
		user_id: props.payout.user_id,
		referral_id: props.payout.referral_id,
		uuid: props.payout.uuid,
		description: props.payout.description,
		amount: props.payout.amount,
		percent: props.payout.percent,
	});
	const save = () => form.put(window.route("payouts.edit", props.payout.id));
</script>
<template>
	<AdminLayout>
		<Head :title="title ?? `Edit Payout`" />
		<main class="h-full">
			<div
